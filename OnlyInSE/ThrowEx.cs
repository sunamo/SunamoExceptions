...
#pragma warning enable
    ...
    {
        ThrowIsNotNull(FullNameOfExecutedCode(T.Item1, T.Item2) +
        $"{variableName} with items {SHSE.JoinNL(item)} is out of range, it is < {isLt} or > {isGt}");
    }

    public static void PassedListInsteadOfArray(string variableName, string[] v)
    {
        ThrowIsNotNull(Exceptions.PassedListInsteadOfArray(FullNameOfExecutedCode(T.Item1, T.Item2), variableName, v));
    }
